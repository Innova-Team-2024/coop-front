        ...
        <ul className="hidden md:flex gap-8 items-center text-sm font-medium relative z-10" ref={dropdownRef}>
          {navItems.map(({ label, key, children }) => (
            <li key={key} className="relative">
              <button
                className={`flex items-center gap-1 transition-colors duration-300 ${openMenu === key ? 'text-black font-semibold' : ''}`}
                onClick={() => setOpenMenu(openMenu === key ? null : key)}
              >
                <span>{label}</span>
                <HiChevronDown
                  size={16}
                  className={`text-gray-500 transition-transform duration-300 ${openMenu === key ? 'rotate-180' : ''}`}
                />
              </button>
              <div
      ...
      {menuOpen && (
        <div className="md:hidden mt-4 bg-white shadow-md rounded-lg px-4 py-3">
          <ul className="flex flex-col gap-4 text-base font-medium">
            ...
            <li className={`cursor-pointer transition-colors duration-300 ${openMobileSubmenus['soporte'] ? 'text-black font-semibold' : ''}`}>Soporte</li>
            <li className={`cursor-pointer transition-colors duration-300 ${openMobileSubmenus['formasdepago'] ? 'text-black font-semibold' : ''}`}>Formas de pago</li>
          </ul>
        </div>
      )}
    </header>
